...
def do_group_splitting(actives,groups,inactives,dic_constants,ntree):
    train_size=0.5
    err = ('','','','','')
    ...
    for spl in zip(a_split,i_split):# iterate across four splits
        ...
        model = get_fit_model(x,y,sw,ntree)
...
def do_tscv(actives,inactives,ntree):
...
def get_model(dic_compounds):
    matrix =  np.vstack((dic_compounds['inactives_train'],dic_compounds['actives_train']))
    label = np.vstack((dic_compounds['inactives_train_y'],dic_compounds['actives_train_y']))
    ntree =  get_optimal_ntree(matrix, label)
    model = RandomForestClassifier(n_jobs=3, n_estimators=ntree, class_weight='balanced',random_state=123)
...
def predict_model(model,x):
    c1 = (np.array(model.predict_proba(x)))[:,1].reshape(-1,1)
    c0 = 1-c1
    arr = np.hstack((c0,c1))
    return arr

def get_optimal_ntree(matrix,label):
    sw = compute_sample_weight('balanced', label)
    ntree,treeinc,max_estimators,conv,idx = 80,10,210,0,0
    clf = RandomForestClassifier(n_jobs=3, warm_start=True, class_weight='balanced',
                                 oob_score=True,random_state=123)
    start_time = time.time()
    elapsed_time = time.time() - start_time
    while (conv < 2) and (elapsed_time <= 60) and (ntree <=max_estimators) == True:
        #while not converged or less than 1min
        ntree+=treeinc
        clf.set_params(n_estimators=ntree)
        clf.fit(matrix,label,sample_weight = sw)
        elapsed_time = time.time() - start_time
        oob_error = 1 - clf.oob_score_
        if idx == 0:
            previous_oob = oob_error
            idx +=1
            continue
        else:
            if oob_error>previous_oob: 
                conv +=1 #if the error increases, then it is not converging
        previous_oob = oob_error
        idx +=1    
...
def conformal_prediction(dic_compounds):
    clf,ntree = get_model(dic_compounds)
...
def do_modelling(actives,pids,scafs,inactives,model_name,model_label,dic_constants):
    ...
    if enough_compounds:
        ...
        start_time = time.time()
        pred_results, model ,ntree= conformal_prediction(dic_compounds)
        elapsed_time = time.time() - start_time
        aprobs_train = model.predict_proba(dic_compounds['actives_train'])[:,1].ravel()
        iprobs_train = model.predict_proba(dic_compounds['inactives_train'])[:,1].ravel()
        aprobs_test = model.predict_proba(dic_compounds['actives_test'])[:,1].ravel()
        iprobs_test = model.predict_proba(dic_compounds['inactives_test'])[:,1].ravel()
        # save model
        with bz2.BZ2File(dic_constants['dir_model_pkls'] + model_name + '.pkl.bz2','w',compresslevel=9) as fid:
        ...
        results_sss =do_group_splitting(actives_train,
                                        ...
                                        ntree)
        ###------TSCV------###
        results_tscv = do_tscv(actives_train,inactives_train,ntree)
...
def initialisation():
    ...
    dir_model_pkls = './RF_pkls_%dto%d/' % (start_id,end_id-1)
    if not os.path.exists(dir_model_pkls):
        os.mkdir(dir_model_pkls)
    
    dir_model_results = './RF_results_%dto%d/' % (start_id,end_id-1)
    if not os.path.exists(dir_model_results):
        os.mkdir(dir_model_results)
        
    dir_training_log = './RF_training_log_%dto%d/' % (start_id,end_id-1)
    if not os.path.exists(dir_training_log):
        os.mkdir(dir_training_log)
    
    dir_model_para = './RF_parameter_search_%dto%d/' % (start_id,end_id-1)
